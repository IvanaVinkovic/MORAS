...
#prvi zadatak a)
        ...
        if l[0] == "push":
            A1 = l[2]
            if len(l) == 3 and int(A1) >= 0 and A1.isdigit():
                return "//" + " ".join(l) + "\n" + self._push(l[1], l[2], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "NEDEFINIRANO");
                return ""

        elif l[0] == "pop":
            A2 = l[2]
            if len(l) == 3 and int(A2) >= 0 and A2.isdigit():
                return "//" + " ".join(l) + "\n" + self._pop(l[1], l[2], n)
            else:
                self._flag = False
                Parser._error("Parser", n, "NEDEFINIRANO");
...
#prvi zadatak b)
    ...
    def _goto(self, lab, n):
        if lab in self.labels: 
            ...
            self._flag = False 
            Parser._error("Goto", n, "Nedefinirani Label")
            return ""
    ...
    def _ifgoto(self, lab, n):
       if lab in self.labels: 
           ...
           self._flag = False
           Parser._error("If-goto", n, "Nedefinirani Label")
           return ""
